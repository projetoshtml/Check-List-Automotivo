...
function setupLogoFallback() {
  const applyFallback = () => {
    if (!dom.logo || dom.logo.tagName !== 'IMG') return;
    dom.logo.removeAttribute('src');
    dom.logo.classList.add('logo-fallback');
    dom.logo.setAttribute('alt', 'TrocÓleo');
    dom.logo.outerHTML = '<div id="brandLogo" class="brand-logo logo-fallback" aria-label="TrocÓleo">TrocÓleo</div>';
    dom.logo = document.getElementById('brandLogo');
  };
...
function createInspectionCard(item, container, withStatus) {
  ...
  state.items[item.id] = state.items[item.id] || {
    photos: [],
    status: withStatus ? 'Não verificado' : '',
    observation: '',
    testPerformed: false
  };
...
function bindMasks() {
  ...
  if (mileageInput) {
    mileageInput.addEventListener('input', () => {
      mileageInput.value = onlyDigits(mileageInput.value);
    });
    mileageInput.addEventListener('blur', () => {
      mileageInput.value = formatMileage(mileageInput.value);
    });
    mileageInput.addEventListener('focus', () => {
      mileageInput.value = onlyDigits(mileageInput.value);
    });
  }
...
function restoreDraft(silent = false) {
  ...
  Array.from(dom.form.elements).forEach(el => {
    if (!el.name) return;
    if (Object.prototype.hasOwnProperty.call(state.form, el.name)) {
      el.value = state.form[el.name];
    }
  });
...
function hydrateItemCard(itemId) {
  ...
  chips.forEach(chip => chip.classList.toggle('active', chip.dataset.value === data.status));
...
function handleNewChecklist() {
  ...
  document.querySelectorAll('.inspection-card').forEach(card => {
    ...
    state.items[itemId] = {
      photos: [],
      status: card.querySelector('.status-group').classList.contains('hidden') ? '' : 'Não verificado',
      observation: '',
      testPerformed: false
    };
...
async function buildPdfBlob() {
  ...
  const addPhotosGrid = async (photos, captionPrefix) => {
    ...
    for (let i = 0; i < photos.length; i++) {
      ...
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(8);
      const caption = `${captionPrefix} ${i + 1}`;
      doc.text(caption, x, rowStart + imgH + 4);
  ...
  if (state.logoDataUrl) {
    doc.addImage(state.logoDataUrl, 'PNG', margin, y, 24, 24, undefined, 'FAST');
  } else {
    doc.setFillColor(15, 39, 79);
    doc.roundedRect(margin, y, 24, 24, 3, 3, 'F');
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(255, 255, 255);
    doc.setFontSize(11);
    doc.text('T', margin + 9, y + 15);
    doc.setTextColor(20, 33, 61);
  }

  doc.setFont('helvetica', 'bold');
  doc.setFontSize(18);
  doc.text('Check List Automotivo', 42, y + 8);
  ...
  for (const item of INTERNAL_ITEMS) {
    ...
    if (item.hasTestCheckbox) {
      doc.text(`Teste realizado: ${itemData.testPerformed ? 'Sim' : 'Não'}`, pageWidth - margin - 3, y + 12, { align: 'right' });
    }
  ...
  if (data.signature) {
    doc.addImage(data.signature, 'PNG', margin + 2, y + 2, pageWidth - margin * 2 - 4, 38, undefined, 'FAST');
  }
...
async function sharePdf() {
  ...
  if (navigator.canShare && navigator.canShare({ files: [file] })) {
    await navigator.share({
      title: 'TrocÓleo Check List Automotivo',
      text: `Checklist ${state.protocol}`,
      files: [file]
    });
    return;
  }
  ...
  saveAs(blob, `${state.protocol}-checklist-trocoleo.pdf`);
  alert('Compartilhamento nativo não disponível neste navegador. O PDF foi baixado para envio manual.');
}
...
async function exportPhotosZip() {
  ...
  if (!hasAnyPhoto) {
    alert('Não há fotos para exportar neste atendimento.');
    return;
  }
